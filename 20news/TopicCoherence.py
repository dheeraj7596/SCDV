...
def get_probability_topic_vectors(word_centroid_map, num_clusters, prob_word):
	...
	for index in range(0, num_clusters):
		...
		topic_centroid_prob_map[index] = {k: (v *1.0)/ (prob_topic[index]) for k, v in (topic_centroid_prob_map[index]).iteritems()}
...
def get_topic_document_prob_map(traindata,word_centroid_prob_map,num_clusters):
	...
	for doc_index in len(traindata):
		...
		topic_document_probability[doc_index] = {k: (v*1.0) / (overall_value) for k, v in (topic_document_probability[doc_index]).iteritems()}
